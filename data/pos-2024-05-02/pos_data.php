...
$action             = $_POST['action'];
$identifier         = 'pos';
$ventas_dia_id      = 'ventas-del-dia';
$pos_inventario_id  = 'pos-inventario';
...
$IS_MATRIZ      = isMatrizType($id_sucursal);
...
switch ($action) {
  ...
  case 'cart-add-item-' . $identifier:
    ...
    if (!$carrito[$id_producto]) :
      ...
      /* if ($data_producto['unidad'] === $TIPO_A_GRANEL) $precio_venta = calculatePricePerBulk(
        $data_producto['contenido'],
        $data_producto['precio_venta'],
        $cantidad
      ); */
  ...
  case 'add-discount-' . $identifier:
    $id_producto                            = cleanStr($_POST['uid']);
    $carrito                                = $_SESSION[$carrito_ssid];
    $carrito_producto_data                  = $carrito[$id_producto];

    $new_price                              = doubleval(cleanStr($_POST['new_price']));
    $discount                               = doubleval(cleanStr($_POST['discount']));

    $carrito_producto_data['precio_venta']  = $new_price;
    $carrito_producto_data['descuento']     = $discount;
    ...
    $response = [
      'status'        => 'success',
      'toastMessage'  => 'El descuento se aplicó correctamente.',
      'callback'      => 'storeCart.loadCart();'
    ];
    break;

  case 'load-' . $ventas_dia_id:
    $have_actions     = haveActions($ventas_dia_id, 'tabla');

    $per_page         = $_POST['perPage'];
    $page             = $_POST['page'];

    $search           = cleanStr($_POST['search']);
    $id_sucursal      = cleanStr($_POST['id_sucursal']) ? cleanStr($_POST['id_sucursal']) : getSessionBranchOfficeId();
    $data_sucursal    = getBranchOfficeData($id_sucursal);
    $fecha            = date('d-m-Y');
    ...
    if ($request['status'] === 'error')   echo getEmptyTableMessage();
    if ($request['status'] === 'success') include $ventas_dia_id . '_table.php';

    echo "
      <script>
        $('#ventas-del-dia-title').html('Ventas del día de {$data_sucursal['nombre_sucursal']}');
      </script>
    ";
    die;
    break;

  case 'action-imprimir-ticket-' . $ventas_dia_id:
    if (checkModuleActionPermission($ventas_dia_id, 'imprimir-ticket')) :
      $uid        = $_POST['uid'];
  ...
  case 'load-' . $pos_inventario_id:
    $have_actions     = haveActions($pos_inventario_id, 'tabla');
    ...
    $by_id_sucursal = "id_sucursal = {$id_sucursal}";
    ...
    if ($request['status'] === 'error')   echo getEmptyTableMessage();
    if ($request['status'] === 'success') include $pos_inventario_id . '_table.php';

    echo "
      <script>
        $('#pos-inventario-title').html('Inventario de {$data_sucursal['nombre_sucursal']}');
      </script>
    ";
    die;
    break;
