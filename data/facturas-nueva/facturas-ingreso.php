...
// obtenemos el folio
$result     = mysqli_query($mysqli, "SELECT MAX(folio) as Num FROM paal_facturas WHERE serie = '{$folioSerie}'");
...
function sellarXML($cfdi, $numero_certificado, $archivo_cer, $archivo_pem)
{
  ...
  // --- CONFIGURACIÓN COMPATIBILIDAD LEGACY ---
...
function generarXML($rfc_emisor)
{
  ...
  $isMultipleSales  = $_POST['isMultipleSales'] ?? 'no';
  ...
  if ($isMultipleSales) :
    /* <cfdi:InformacionGlobal Periodicidad="01" Meses="12" Año="2025"/> */

    $periodicidad = $_POST['fg_periodicidad'] ?? '';
    $meses        = $_POST['fg_meses'] ?? '';
    $anio         = $_POST['fg_anio'] ?? '';

    if ($periodicidad != '' && $meses != '' && $anio != '') {
      $xml->startElementNs('cfdi', 'InformacionGlobal', null);
      $xml->writeAttribute('Periodicidad', $periodicidad);
      $xml->writeAttribute('Meses', $meses);
      $xml->writeAttribute('Año', $anio);
      $xml->endElement();
    }
  endif;
  ...
  if (isset($_POST['uidVenta']) and $_POST['uidVenta'] != '') {
    ...
    foreach ($productos as $producto) :
      ...
      $valorUnitario  = doubleval($producto['priceWithoutIVA'] ?? 0);
      $descuento      = doubleval($producto["discount"] ?? 0);
      $importe        = doubleval($producto['amountWithoutIVA'] ?? 0);
      $ieps           = doubleval($producto['iepsCurrency'] ?? 0);
      $iva            = doubleval($producto['ivaCurrency'] ?? 0);
      $iepsRate       = doubleval($producto['iepsPercentage'] ?? 0) / 100;
      $ivaRate        = ($iva > 0) ? doubleval($producto['iva'] ?? 16) / 100 : 0;
      $baseSinImpuestos = max(0, $importe - $descuento);
      $baseIVA = $baseSinImpuestos + $ieps;
      $iepsPercentaje = $producto["iepsPercentaje"] ?? 8;
      $ivaPercentaje  = $producto["ivaPercentaje"] ?? 16;
      ...
      if ($isMultipleSales == "si") :
        $xml->writeAttribute('NoIdentificacion', $producto['saleFolio']);
      endif;
      ...
      if ($ieps > 0 || $iva > 0) :
        ...
        if ($ieps > 0) {
          // Calcular la bse a partir del importe
          $iepsBase = (100 * $ieps) / $iepsPercentaje;
          $iepsRate = $iepsPercentaje / 100;

          $xml->startElementNs('cfdi', 'Traslado', null);
          $xml->writeAttribute('Base', $formatSatDecimals($iepsBase));
          $xml->writeAttribute('Impuesto', '003');
          $xml->writeAttribute('TipoFactor', 'Tasa');
          $xml->writeAttribute('TasaOCuota', number_format($iepsRate, 6, '.', ''));
          $xml->writeAttribute('Importe', $formatSatDecimals($ieps));
          ...
          $addTrasladoResumen('003', $iepsRate, $iepsBase, $ieps);
        }

        if ($iva > 0) {
          // Calcular la bse a partir del importe
          $ivaBase = (100 * $iva) / $ivaPercentaje;
          $ivaRate = $ivaPercentaje / 100;

          $xml->startElementNs('cfdi', 'Traslado', null);
          $xml->writeAttribute('Base', $formatSatDecimals($ivaBase));
          $xml->writeAttribute('Impuesto', '002');
          $xml->writeAttribute('TipoFactor', 'Tasa');
          $xml->writeAttribute('TasaOCuota', number_format($ivaRate, 6, '.', ''));
          $xml->writeAttribute('Importe', $formatSatDecimals($iva));
          ...
          $addTrasladoResumen('002', $ivaRate, $ivaBase, $iva);
        }
...
function GenerarPDF($comprobante, $numero_certificado, $rfc_emisor, $nombreComprobante)
{
  ...
  /* $pdf->Cell(80, 5, "NOTA DE CRÉDITO", 1, 0, "L", false);
  $pdf->Ln(5);

  $pdf->Cell(80, 5, "COMPROBANTE FISCAL DIGITAL", 1, 0, "L", false);
  $pdf->Ln(5); */
  ...
  if ($tipoFactura == "E") :
    $pdf->Cell(80, 5, utf8_decode("NOTA DE CRÉDITO"), 1, 0, "L", false);
    $pdf->Ln(5);
  else :
    $pdf->Cell(80, 5, utf8_decode("FACTURA"), 1, 0, "L", false);
    $pdf->Ln(5);
  endif;
  ...
  /*$pdf->SetFont('Arial','',8);
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetFillColor(224,224,224);
        $pdf->Cell(80, 5,"RECIBO",1,0,"L",true);
        $pdf->Ln(5);
        $pdf->SetFont('Arial','',7);
        $pdf->SetX(120);
        $pdf->SetTextColor(0,0,0);
        $pdf->Cell(80, 5,'seriado',1,0,"L");
        $pdf->Ln(5);*/
  $pdf->SetFont('Arial', '', 8);
  $pdf->SetX($rightBoxX);
  $pdf->SetTextColor(0, 0, 0);
  ...
  $pdf->SetX($rightBoxX);
  $pdf->SetTextColor(0, 0, 0);
  //$pdf->Cell($rightBoxWidth, 5,utf8_decode($emisor['Municipio']).", ".utf8_decode($emisor['Estado']).", ".date('Y-m-d'),1,0,"L");
  ...
  if ($isMultipleSales == "si") {
    ...
    $periodicidadLabel = FACTURA_GLOBAL_PERIODICIDAD[$periodicidad] ?? '';
    $mesesLabel = FACTURA_GLOBAL_MESES[$meses] ?? '';

    $pdf->SetFont('Arial', '', 8);
    $pdf->SetFillColor(224, 224, 224);
    $pdf->Cell(0, 5, utf8_decode("INFORMACIÓN GLOBAL"), 1, 0, "L", true);
    $pdf->Ln(5);

    $pdf->SetFont('Arial', '', 7);
    $pdf->SetFillColor(255, 255, 255);
    $pdf->Cell(50, 5, utf8_decode("Periodicidad: " . $periodicidadLabel), 0, 0, "L");
    $pdf->Cell(50, 5, utf8_decode("Mes: " . $mesesLabel), 0, 0, "L");
    $pdf->Cell(50, 5, utf8_decode("Año: " . $anio), 0, 0, "L");
    $pdf->Ln(5);
  }
  ...
  if ($isMultipleSales == "si") $tableHeader = [
    ["Cant", 15, "C"],
    ["Unidad", 20, "C"],
    ["Cve. Prod", 25, "C"],
    [utf8_decode("No. Identificación"), 35, "C"],
    [utf8_decode("Descripción"), 35, "C"],
    ["Cve. Sat", 15, "C"],
    ["Precio unit.", 25, "R"],
    ["Importe", 20, "R"]
  ];
  ...
  foreach ($tableHeader as $header) {
    $pdf->Cell($header[1], 5, $header[0], 1, 0, $header[2], true);
  }
  ...
  if (isset($_POST['uidVenta']) and $_POST['uidVenta'] != '') {
    ...
    foreach ($productos as $producto) :
      ...
      if ($comentarios != '') $descripcion .= "\n" . $comentarios;

      // Actualizar el comentario del producto de venta
      $idVenta = $_POST["idVenta"];

      if ($idVenta) {
        $query = "UPDATE paal_venta_productos SET comentarios = '{$comentarios}' WHERE id_venta = {$idVenta} AND id_producto = {$productId}";
        error_log("Updating product comments: " . $query);
      ...
      if ($isMultipleSales == "si") $tableRow = [
        utf8_decode($cantidad),
        utf8_decode($claveUnidad),
        utf8_decode($claveProdServ),
        $producto["saleFolio"],
  ...
  if ($tipo_relacion != '' && count($cfdi_relacionado) > 0) {
    ...
    $pdf->MultiCell(0, 4, utf8_decode("Tipo de Relación: " . $tipo_relacion_text), 0, "L");
    $cfdi_relacionado_text = implode(", ", $cfdi_relacionado);
    $pdf->MultiCell(0, 4, utf8_decode("CFDIs Relacionados: " . $cfdi_relacionado_text), 0, "L");
    $pdf->Ln(4);
  }
  ...
  if (trim($comentarios) != '') {
    $pdf->Ln(4);
    $pdf->MultiCell(0, 4, utf8_decode("Comentarios: " . $comentarios), 0, "L");
  }
  ...
  //$pdf->SetY(145);
  // Ancho de página menos margenes
  ...
  $fecha          = date('Y-m-d', strtotime($_POST['fecha_emision'])) . " " . date('H:i:s');
   ...
   * Insertar la factura
   */
  $invoiceModel = new FacturasIngresoModel();
  ...
  if ($result->status == "success" && $saleUids) {
    ...
    foreach ($saleUids as $saleUid) {
      ...
      $saleInvoicesModel->setType("ingreso");
      $saleInvoicesModel->create();
    }
  }

  if (isset($_POST['enviar_al_correo']) && $_POST['enviar_al_correo'] == 'si' && trim($_POST['correo']) != '') :
    $sender       = new stdClass();
