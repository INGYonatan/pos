...
const FCS_CARPETA_URL   = CARPETA_FACTURAS_ANTICIPO_DE_COMPRA_URL;
const FCS_CARPETA_PATH  = CARPETA_FACTURAS_ANTICIPO_DE_COMPRA_PATH;

$result     = mysqli_query($mysqli, "SELECT MAX(folio) as Num FROM paal_facturas_anticipo_compra WHERE serie = '{$folioSerie}'");
...
function GenerarPDF($comprobante, $numero_certificado, $rfc_emisor, $nombreComprobante)
{
  ...
  if ($tipoFactura == "E") :
    $pdf->Cell($rightBoxWidth, 5, utf8_decode("NOTA DE CRÉDITO"), 1, 0, "L", false);
    $pdf->Ln(5);
  else :
    $pdf->Cell($rightBoxWidth, 5, utf8_decode("FACTURA"), 1, 0, "L", false);
    $pdf->Ln(5);
  endif;

  $pdf->SetXY($rightBoxX, 10);
  $pdf->SetFont('Arial', 'B', 8);
  $pdf->Cell($rightBoxWidth, 5, "COMPROBANTE FISCAL DIGITAL", 1, 0, "L", false);
  $pdf->Ln(10);
  ...
  /*$pdf->SetFont('Arial','',8);
        ...
        $pdf->SetTextColor(0,0,0);
        $pdf->SetFillColor(224,224,224);
        $pdf->Cell($rightBoxWidth, 5,"RECIBO",1,0,"L",true);
        $pdf->Ln(5);
        $pdf->SetFont('Arial','',7);
        $pdf->SetX($rightBoxX);
        $pdf->SetTextColor(0,0,0);
  ...
  $pdf->Cell($leftInfoWidth, $cellHeight, "Cantidad con letra:  " . numtoletras($total), 0, 0, "L");

  $pdf->SetXY($rightCellX, $summaryStartY);
  $pdf->Cell($leftCellWidth, $cellHeight, "Subtotal:", 1, 0, "R");
  $pdf->Cell($rightCellWidth, $cellHeight, "$" . number_format($subtotal, DECIMALS_CURRENCY, ".", ","), 1, 0, "R");
   ...
   * Insertar la factura
   */
  $invoiceModel = new FacturasAnticipoCompraModel();
  ...
  if ($result->status == "success" && $saleUids) {
    ...
    foreach ($saleUids as $saleUid) {
      ...
      $saleInvoicesModel->setType("anticipo");
