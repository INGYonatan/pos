    ...
    <table class="table table-sm table-hover">
      ...
      <tbody>
        ...
        ?>
          <tr class="pos-tr">
            ...
            <th class="text-center">
              ...
              } elseif ($row['tipo'] === 'rines') {
                $badge_color = 'bg-info';
              } elseif ($row['tipo'] === 'refacciones') {
                $badge_color = 'bg-warning';
              } elseif ($row['tipo'] === 'servicios') {
              ...
              <span class="badge <?= $badge_color; ?> text-capitalize"><?= $row['tipo']; ?></span>

              <?php if ($show_serial) : ?>
                <br>
                <a class="btn-serial-numbers btn btn-xs btn-light border rounded text-dark" data-bs-toggle="modal" data-serialNumbers="<?= htmlentities(json_encode($serial_numbers)); ?>" data-itemId="<?= $id; ?>" data-branchId="<?= $id_sucursal; ?>" href="#modal-serialNumbers">Nros. serie</a>
              <?php endif; ?>
            </th>
            ...
            <td class="text-center">
              <?= format_decimal_number($ieps_percentage, DECIMALS_CURRENCY); ?>%
            </td>

            <td class="text-center">
              <?= format_decimal_number($iva_percentaje, DECIMALS_CURRENCY); ?>%
            </td>

            <td class="text-end">
              <span id="cost-price-label-<?= $id; ?>">$<?= number_format(($unit_total), DECIMALS_CURRENCY); ?></span>

              <a id="cart-row-action-<?= $identifier; ?>-<?= $id; ?>" class="pos-btn cart-row-action btn-update-row ms-1 me-1 pulse" data-itemId="<?= $id; ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="Actualizar fila" href="javascript:void(0)">
        ...
        <tr>
          ...
          <td class="text-end">$<?= number_format($subtotal, DECIMALS_CURRENCY); ?></td>
        </tr>

        <tr>
          <td colspan="9" class="text-end fw-bold">IEPS:</td>
          <td class="text-end">$<?= number_format($total_ieps, DECIMALS_CURRENCY); ?></td>
        </tr>

        <tr>
          <td colspan="9" class="text-end fw-bold">IVA:</td>
          <td class="text-end">$<?= number_format($total_iva, DECIMALS_CURRENCY); ?></td>
        </tr>

        <tr>
          <td colspan="9" class="text-end fw-bold">Total:</td>
          <td class="text-end">$<?= number_format($total, DECIMALS_CURRENCY); ?></td>
        </tr>
      </tbody>
    </table>
  </div>
