...
<?php if (!isEmptyArray($list)) : ?>
  <table class="table table-hover table-striped">
    ...
    <tbody>
      <?php foreach ($list as $product) :
        ...
        $serialNumberCatalog = getProductSerialNumbers($id, $branch_id);

        #
        $inventoryControl = $product->inventory_control;
      ...
      ?>
        ...
        <tr class="pos-tr">
          ...
          <td class="text-center">
            ...
            <?php if ($product->requires_serial_number) : ?>
              <br>
              <a class="btn-serial-numbers btn btn-xs btn-light border rounded text-dark" data-bs-toggle="modal" data-serialNumbers="<?= htmlentities(json_encode($serial_numbers)); ?>" data-serialNumberCatalog="<?= htmlentities(json_encode($serialNumberCatalog)); ?>" data-itemId="<?= $id; ?>" data-branchId="<?= $branch_id; ?>" href="#modal-serialNumbers">Nros. serie</a>
            <?php endif; ?>
          </td>

          <td class="text-center">
            <?php if ($inventoryControl == "si") : ?>
              <?php if ($stock <= 0) : ?>
                <span class="badge btn-light text-danger">Sin stock</span>
                <span class="without-stock-controller"></span>
              <?php endif; ?>
            ...
            <form id="quantity-<?= $id; ?>-form" class="d-flex align-items-center gap-1 quantity-form" autocomplete="off">
              <div class="form-group form-group-sm m-0">
                <div class="input-group">
                  <input class="form-control form-control-sm" name="quantity" min="1" <?= $inventoryControl == "si" ? "max='{$stock}'" : ""; ?> value="<?= $quantity; ?>" type="number">
          ...
          <tr>
            ...
            <td class="text-end align-middle">
              <div class="form-group m-0 ms-auto" style="max-width: 5rem;">
                <input id="cartRounding" class="form-control form-control-sm" value="<?= round($cart_session->sale_rounding, DECIMALS_CURRENCY_TICKET); ?>" step="<?= DECIMALS_CURRENCY_TICKET; ?>" min="0" type="number">
              </div>
            </td>
          </tr>
  ...
  <script>
    ...
    if ($(".without-stock-controller").length > 0) {
      $("#btn-pos").attr("disabled", true);

      // Agregar un Alert bootstrap arriba del botón de Cobrar venta
      const alertHtml = `<div class="alert alert-danger alert-dismissible fade show" role="alert">
        <strong>Atención!</strong> Hay productos sin stock en el carrito. No se puede procesar la venta.
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
      </div>`;

      $(alertHtml).insertBefore("#btn-pos");
    }
