...
switch ($action):
  ...
  case "cart-clean-cart-{$identifier}":
    ...
    $response->toastMessage = 'El carrito se vació correctamente';
    break;
  ...
  case "cart-save-cart-{$identifier}":
    try {
      ...
      if (!isEmptyArray($list)) :
        ...
        $folio        = get_quote_folio($branch_id);
        ...
        if ($query_result) :
          ...
          $response->status       = 'success';
          $response->toastMessage = 'La cotización se realizó correctamente';
          $response->pdf          = $pdf;
        endif;
      endif;
    } catch (Exception $e) {
...
// $response->toastMessage = json_encode($_POST);
