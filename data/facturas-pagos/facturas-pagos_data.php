...
function cancelarFactura(
  ...
  if ($cliente->cancelar($UUID, $opciones)) {
    ...
    mysqli_query($mysqli, "UPDATE paal_facturas_p SET cancelado = 1 WHERE id_factura= '" . $idFactura . "'");
...
switch ($action) {
  case 'load-' . $identifier:
    ...
    $c_from           = "{$db_dti}_facturas_p AS F";
    $c_extra_clauses  = "ORDER BY F.id_factura DESC";

    $fields = [
      "F.id_factura",
      ["F.id_factura", "uid"],
      "F.id_factura_ingreso",
      "F.id_emisor",
      "F.id_cliente",
      "F.serie",
      "F.folio",
      ...
      ["(CONCAT(FI.serie, '-', FI.folio))", "folio_ingreso"],
      "FPP.monto",
    ...
    $c_join = "
      ...
      LEFT JOIN {$db_dti}_facturas AS FI  ON (FI.id_factura = F.id_factura_ingreso)
      RIGHT JOIN {$db_dti}_facturas_p_pagos AS FPP ON (FPP.id_factura = F.id_factura)
      LEFT JOIN {$db_dti}_formas_pago AS FP ON (FP.id = FPP.id_forma_pago)
    ...
    $filtersSearch = [
      ...
      ["FI.serie",  "%$search%", "LIKE", "OR"],
      ["FI.folio",  "%$search%", "LIKE", "OR"],
      ["CONCAT(FI.serie, '-', FI.folio)", "%$search%", "LIKE", "OR"],
      ["C.nombre_completo",  "%$search%", "LIKE", "OR"],
      ["E.nombre_razon_social",  "%$search%", "LIKE", "OR"],
      ["FPP.monto",  "%$search%", "LIKE", "OR"],
      ["FP.descripcion",  "%$search%", "LIKE", "OR"],
    ];
    ...
    if (!empty($billId)) array_push($c_where, ["(MD5(F.id_factura_ingreso))", $billId]);
