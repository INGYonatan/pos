...
switch ($action):
  ...
  case "cart-clean-cart-{$identifier}":
    $cart = get_quote_data($quote_id, $branch_id);

    $_SESSION[$carrito_ssid] = $cart;

    $response->status = 'success';

    if ($_POST['useAlert'] != 'false') $response->toastMessage = 'El carrito se reinició correctamente';
  ...
  case "cart-save-cart-{$identifier}":
    ...
    if (!isEmptyArray($list)) :
      ...
      $exchange               = cleanStr($_POST['exchange']);
      $observations           = "Cotización convertida a venta";
      ...
      if ($pay_with < $total) :
        $response->toastMessage = "El pago no está completo";
        echo json_encode($response);
        die;
      endif;
      ...
      if ($cash <= 0) :
        ...
        if ($pay_with > $total) :
          $response->toastMessage = "El pago tiene que ser la cantidad exacta de {$symbol}{$total_format}";
          echo json_encode($response);
          die;
        endif;
      endif;
