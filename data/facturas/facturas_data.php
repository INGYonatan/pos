...
function cancelarFactura(
  ...
  // error_log("DATOS DE CONEXIÓN: url: $url_timbrado, user_id: $user_id, user_password: $user_password");
  ...
  error_log("Intentando cancelar factura UUID: $UUID con motivo: $motivo y folio sustituto: $folioSustituto");

  $resCancelar = $cliente->cancelar($UUID, $opciones);
...
switch ($action) {
  case 'load-' . $identifier:
    ...
    $c_extra_clauses  = "GROUP BY F.id_factura ORDER BY F.id_factura DESC";
  ...
  case "cancelar-factura":
    ...
    if (!$idFactura) {
      $response["toastMessage"] = "¡ID de factura no proporcionado!";
      echo json_encode($response);
      die;
    }

    if ($idFactura) :
      error_log("Inicia proceso de cancelación de factura ID: $idFactura");
      ...
      if ($numRows > 0) :
        error_log("Factura encontrada, procediendo a cancelar...");
        ...
        error_log("Resultado de la cancelación: " . print_r($result, true));
