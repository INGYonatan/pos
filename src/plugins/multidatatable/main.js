class MultiDataTable {
  ...
  _loadDataTable = (tableIdentifier, tablePage) => {
    ...
    //this.state.location ? this.state.location : this.state.identifier;

    let location = this.state.identifier;
    if (this.state.locations[tableIdentifier]) location = this.state.locations[tableIdentifier];
    if (this.state.location) location = this.state.location;
  ...
  _initDataTable = () => {
    ...
    if (this.state.tablesConfig) {
      ...
      for (const key in tablesConfig) {
        ...
        const location = config?.location ? config.location : null;

        if (location) this.state.locations[identifier] = location;

        this.startConfigTable(identifier, location);
      }
    }
  }
  ...
  startConfigTable = (identifier) => {
    ...
    $(`${form} select:not([data-nofilter])`).on('change', (e) => {
      e.preventDefault();
      console.log('filter changed');
      load(1);
    });
  ...
  _handlePressAction = async (identifier, data, action) => {
    ...
    if (action?.actionValue) actionValue = action.actionValue;
    ...
    if (action?.parameters) {
      for (const valueName in action.parameters) {
        parameters[valueName] = action.parameters[valueName];
      }
    }
