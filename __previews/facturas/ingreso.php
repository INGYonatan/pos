...
function GenerarPDF()
{
  ...
  //$pdf->MultiCell(110, 5, utf8_decode($receptorNom  bre) . "\n" . utf8_decode($receptorRFC) . "\n" . utf8_decode('  Uso del CFDI: ' . $receptorUsoCFDIPDF), 0, "L");
  //$pdf->SetXY(0, 65);
  // $pdf->SetY(85);
  // BLoque para poner datos globales en una factura global
  ...
  */
  ...
  if ($tipo_relacion != '' && count($cfdi_relacionado) > 0) {
    ...
    $pdf->MultiCell(0, 5, utf8_decode("Tipo de Relación: " . $tipo_relacion_text), 0, "L");
    $cfdi_relacionado_text = implode(", ", $cfdi_relacionado);
    $pdf->MultiCell(0, 5, utf8_decode("CFDIs Relacionados: " . $cfdi_relacionado_text), 0, "L");
    $pdf->Ln(5);
  }
